...
def main():
    ...
    ############################################################################
    """
    # h4.cmd('tcpreplay -i eth0 -l 100 -M 40 ./h4_eth0_4Gb_ttl_8.pcap &') # replay the traffic
    # h0.cmd('sar -n DEV 1 100 > ./h0_sar.log &')

    # print("Start the Testing...")
    h4.cmd("python3 ./dctrace/trace_send.py -i 0 &")
    for i in [1, 2, 3, 9]:
        h = net.get(topo.id2P4[i])
        h.cmd("python3 ./dctrace/trace_receive.py -i %d &" % i)
