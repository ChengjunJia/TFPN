...
//   We combine tha destination during the path finding
public class atpgOnlyEndhostCombineDst {
    public static void main(String args[]) throws SecurityException, IOException {
        ...
        ArrayList<String> loggerNames = new ArrayList<String>(
                Arrays.asList("run_stanford_only_endhost_combine_dst.log", "run_fattree_2_only_endhost_combine_dst.log",
                        "run_fattree_4_only_endhost_combine_dst.log"));
        int bitSize = 34;
        if (inputID == 0) {
        ...
        for (int i = 0; i < netPortList.size(); i++) {
            ...
            Ports.remove(netPortList.get(i)); // Add all ports but remove the local to avoid loop!
            ...
            logger.info("Node " + netPortList.get(i) + "->*, path num: " + result.size());
            // if (result.size() == 0) {
            // continue;
            // }
            // IGNORE THE OUTPUT
        ...
        logger.info("Network ports number is " + netPortSet.size() + "; link# is " + topoRules.size() + "; rule# is "
