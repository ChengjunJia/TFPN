 ...
 *
 */
public class BDDACLWrappertesttest implements Serializable{
	...
	public int encodeACLin(ArrayList<Subnet> subs, int rawBDD, int reftimes)
	{
		...
		int destipbdd = encodeDstIPPrefixs(subs);//destipbdd=multi-preifx's "or" of one sub
	...
	public int encodeDstIPPrefixs(ArrayList<Subnet> subs)
	{
		...
		for(int i = 0; i < subs.size(); i ++)
		{
			...
			int dstipbdd = encodeDstIPPrefix(onesub.getipaddr(), onesub.getprefixlen());//compute the BDD of the subs
			int tmp = aclBDD.or(res, dstipbdd);//multi-preifx's "or" of one sub
			aclBDD.ref(tmp);
			aclBDD.deref(res);
	...
	public int ConvertACLs(LinkedList<ACLRule> acls)
	{
		...
		for(int i = 0; i < acls.size(); i ++)
		{
			...
			int g = ConvertACLRule(acl);//output aclr's bdd, no not consider about the override

			if(PacketSet.CheckPermit(acl))//compute the deny and permit influence to the acl's bdd
			{
				if(res == BDDFalse)
				{
	 ...
	 */
	public int ConvertACLRule(ACLRule aclr)//output aclr's bdd, no not consider about the override
	{	
		/**
		 *  protocol
		 */
		...
		if(aclr.protocolLower == null || 
				aclr.protocolLower.equalsIgnoreCase("any"))//"equalsignorecase" is similar to equals,ignore the distinguish between big and smale label  
		{
			//do nothing, just a shortcut
		}else{
			Range r = PacketSet.convertProtocolToRange
					(aclr.protocolLower, aclr.protocolUpper);
			protocolNode = ConvertProtocol(r);//protocolNode's bdd
		}
		...
		int [] fiveFields = {protocolNode,srcPortNode,dstPortNode,
				srcIPNode,dstIPNode};
		int tempnode = AndInBatch(fiveFields);//five tuples bits in total
